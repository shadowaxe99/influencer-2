...
const SettingsContent = ({ closeSettings }) => {
    ...
    const handleScale = (input, value) => {
        ...
        if (
            ...
            const middle =
                (document.body.scrollHeight / 2) * (Number(value) - 1);
    ...
    const handleHideBackgroundSelect = (input) => {
        config.updateHideBackgroundSelect(!input.value);
        input.onChange(!input.value);
    };

    const handleHideSessionSelect = (input) => {
        ...
        <>
            ...
            <Box display="flex" ml="6.4rem" mt="0.5rem">
                    ...
                    {({ input }) => {
                        return (
                            <Button
                                ...
                                onClick={() =>
                                    handleHideBackgroundSelect(input)
                                }
                            />
                        );
                    }}
                ...
                <Typography sx={checkboxLabelStyle}>
                    Hide Background Select
                </Typography>
            </Box>
            <Box display="flex" ml="6.4rem" mt="0.2rem">
                <Field
                    name="hideSessionSelect"
                    initialValue={config.hideSessionSelect}
                >
                    {({ input }) => {
                        return (
                            <Button
