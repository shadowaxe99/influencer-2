...
function App() {
  ...
  const predefinedMessages = [
    "Hello! Let's schedule your feedback appointment.",
    "Are you available tomorrow at 10 AM?",
    "Great! I've scheduled you for tomorrow at 10 AM.",
  ...
  useEffect(() => {
    if (submitted) {
      setSchedules([
        "Meeting with HR - Tomorrow, 9 AM",
        "Technical Interview - Tomorrow, 11 AM",
        "Lunch Break - Tomorrow, 1 PM"
      ]);

      let i = 0;
      const interval = setInterval(() => {
        if (i < predefinedMessages.length) {
          setMessages(prev => [...prev, predefinedMessages[i]]);
          i++;
        } else {
          clearInterval(interval);
          setSchedules(prev => [...prev, "Feedback Session - Tomorrow, 10 AM"]);
        }
      }, 2000);
      return () => clearInterval(interval);
    }
