...
def run_command_in_new_terminal(command):
    ...
    if command.startswith("python "):
        ...
        command = os.path.join(current_dir, command)
    elif command.startswith("xcrun "):
        command = command.replace("xcrun ", "open ", 1)
        command = os.path.join(current_dir, "pet-therapy-main", "Sources", "macOS", command)
